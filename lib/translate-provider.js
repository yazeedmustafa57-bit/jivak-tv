...
const BADINI_KU_LINE_MAX = 280
const BADINI_KU_REQUEST_MAX = 480
...
function mapLangForBadini(code) {
  // Der Badini-Proxy (translator-site-five) unterstützt "ku" (Google Kurdisch),
  // aber nicht "badini" als Sprachcode. "ku" liefert echte Kurdisch-Übersetzungen.
  return code === 'ku' ? 'ku' : code
}
...
async function translateBadiniText(text, from, to) {
  ...
  const results = await pMap(
    ...
    (req) => {
      ...
      return badiniRequestWithRetry(requestText, req.sections.length, from, to)
    },
    BADINI_KU_CONCURRENCY
  )
...
async function translateWithBadini(text, from, to) {
  ...
  if (badiniMode) {
    const [title, intro, body] = await Promise.all([
      badiniRequestWithRetry(String(text?.title || '').trim(), 1, from, to).catch(() => String(text?.title || '').trim()),
